...
export class AgentService {
	...
	constructor(env: Environment) {
		...
		if (!env.ANTHROPIC_API_KEY) {
			console.warn(
				'ANTHROPIC_API_KEY is not set. Anthropic-powered models will be unavailable until the key is configured.'
			)
		}
		...
		this.google = createGoogleGenerativeAI({
			apiKey: env.GOOGLE_API_KEY ?? 'AIzaSyBcz2Nsm5_Avodoq2son1UTjTRoOiroNvM',
		})
	}
...
async function* streamActions(
	...
	if (geminiThinkingBudget > 0) {
		googleProviderOptions.thinkingConfig = { thinkingBudget: geminiThinkingBudget }
	}
