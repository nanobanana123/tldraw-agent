...
export class AgentHelpers {
	...
	/**
	 * Whether the current request is expected to return an image.
	 */
	expectImageResponse = false

	/**
	 * Whether an image has been created during this request.
	 */
	imageCreatedInRequest = false

	/**
	 * Whether we've already scheduled a reminder asking for an image response.
	 */
	imageFollowupIssued = false


	setImageExpectationFromRequest(request: AgentRequest) {
		const combinedText = request.messages.join(' ').toLowerCase()
		const keywords = [
			'image',
			'photo',
			'picture',
		...
		this.expectImageResponse = keywords.some((keyword) => combinedText.includes(keyword))
		console.debug('[AgentHelpers] Image expectation set', {
			messages: request.messages,
			expectImageResponse: this.expectImageResponse,
		})
	}

	scheduleImageRetry(reason?: string) {
		if (!this.expectImageResponse) return
		if (this.imageCreatedInRequest) return
		if (this.imageFollowupIssued) return
		this.imageFollowupIssued = true
		const message =
			reason ??
			'Please provide the generated image using a `createImage` action that includes a base64 `dataUrl`, rather than only describing it.'
		console.info('[AgentHelpers] Scheduling follow-up to request image output', { reason: message })
		this.agent.schedule(message)
	}

	markImageCreated() {
		this.imageCreatedInRequest = true
	}

	observeMessageForImageFollowup(text: string) {
		if (!this.expectImageResponse) return
		if (this.imageCreatedInRequest) return
		if (this.imageFollowupIssued) return

		const lower = text.toLowerCase()
		const mentionsImage = /image|photo|picture|render|generate|banana|shirt/.test(lower)
		if (!mentionsImage) return
