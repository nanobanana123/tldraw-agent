...
function DiffSteps({ steps }: { steps: DiffStep[] }) {
	...
	return (
		<div className="agent-changes">
			{steps.map((step, i) => {
				const shouldRenderDescription = step.description && step.description !== previousDescription
				if (shouldRenderDescription) {
					previousDescription = step.description ?? ''
				}

				if (!shouldRenderDescription && !step.imagePreview) {
					return null
				}
				...
				return (
					<div className="agent-change" key={'intent-' + i}>
						{shouldRenderDescription && (
							<div className="agent-change-text">
								{step.icon && (
						...
						{step.imagePreview && (
							<div className="agent-change-image-preview">
								<img
									src={step.imagePreview.dataUrl}
									alt={step.imagePreview.altText}
									loading="lazy"
								/>
							</div>
						)}
